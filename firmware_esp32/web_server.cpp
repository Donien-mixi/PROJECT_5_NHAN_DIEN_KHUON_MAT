...
<body>
    <div class="container">
        <h1>🚀 ESP32-S3 TinyML Face Attendance (OV5640)</h1>
    ...
    <script>
        ...
        // Tự động làm mới luồng ảnh trực tiếp mượt mà (~12 FPS) không block HTTP server
...
static esp_err_t status_handler(httpd_req_t *req) {
    ...
    snprintf(json, sizeof(json),
             "{\"name\":\"%s\",\"sim\":%.3f,\"matched\":%s,\"heap\":%u,\"psram\":%u,\"uptime\":%lu}",
...
static esp_err_t attendance_handler(httpd_req_t *req) {
    File f = SPIFFS.open("/attendance.csv", FILE_READ);
    if (!f) {
    ...
    char chunk[256];
    while (f.available()) {
        int bytes = f.read((uint8_t*)chunk, sizeof(chunk));
        if (bytes > 0) {
...
void setup_web_server() {
    ...
    int retry = 0;
    while (WiFi.status() != WL_CONNECTED && retry < 16) { // Timeout sau 4s
        delay(250);
        Serial.print(".");
        retry++;
    }
    ...
    if (WiFi.status() != WL_CONNECTED) {
        Serial.println("\n⚠️ [WiFi] Khong the ket noi Wi-Fi! He thong chay 100% OFFLINE doc lap.");
        return;
    }

    Serial.println("\n✅ [WiFi] Da ket noi thanh cong!");
    Serial.print("👉 Dia chi IP ESP32: http://");
    Serial.println(WiFi.localIP());

    if (!s_stream_buf) {
        s_stream_buf = (uint8_t*)heap_caps_malloc(STREAM_BUF_SIZE, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
        if (!s_stream_buf) {
            Serial.println("❌ LỖI: Không thể cấp phát s_stream_buf trên PSRAM!");
            return;
        }
    }
    ...
    config.stack_size = 12288; // Tăng stack size an toàn cho HTTP server
    ...
    if (httpd_start(&s_httpd, &config) == ESP_OK) {
        ...
        Serial.println("🌐 [Web Server] HTTP Server da khoi dong tren port 80!");
    } else {
        Serial.println("❌ [Web Server] Khoi dong that bai!");
    }
}
