...
void setup_face_detector(){
    ...
    // Trong ESP-IDF với component esp-tflite-micro, AddConv2D/AddDepthwiseConv2D
    // tự động chạy kernel assembly SIMD esp-nn (-DESP_NN)
    resolver.AddConv2D();
    ...
    static tflite::MicroInterpreter interp(detector_model, resolver, g_detector_tensor_arena, DETECTOR_ARENA_SIZE);
    detector_interpreter = &interp;
    if(detector_interpreter->AllocateTensors() != kTfLiteOk){
        ESP_LOGE(TAG, "[Detector] AllocateTensors FAILED");
        return;
    }
    ESP_LOGI(TAG, "[Detector] Arena used %d / %d bytes (PSRAM, esp-nn SIMD: BẬT)",
             (int)detector_interpreter->arena_used_bytes(), DETECTOR_ARENA_SIZE);
...
FaceBox detect_face(){
    ...
    for(int y = 0; y < RAW_FRAME_SIZE; ++y){
        for(int x = 0; x < RAW_FRAME_SIZE; ++x){
            ...
            if(detector_input->type == kTfLiteFloat32){
                ...
                detector_input->data.int8[idx + 0] = (int8_t)std::max(-128, std::min(127, (int)roundf(rf / input_scale + input_zero_point)));
                detector_input->data.int8[idx + 1] = (int8_t)std::max(-128, std::min(127, (int)roundf(gf / input_scale + input_zero_point)));
                detector_input->data.int8[idx + 2] = (int8_t)std::max(-128, std::min(127, (int)roundf(bf / input_scale + input_zero_point)));
            } else if(detector_input->type == kTfLiteUInt8 && input_scale > 0.0f){
                detector_input->data.uint8[idx + 0] = (uint8_t)std::max(0, std::min(255, (int)roundf(rf / input_scale + input_zero_point)));
                detector_input->data.uint8[idx + 1] = (uint8_t)std::max(0, std::min(255, (int)roundf(gf / input_scale + input_zero_point)));
                detector_input->data.uint8[idx + 2] = (uint8_t)std::max(0, std::min(255, (int)roundf(bf / input_scale + input_zero_point)));
            } else {
                return box;
            }
        }
    ...
    const float crop_size = std::max(g_ema_w, g_ema_h);
    const float left = std::max(0.0f, g_ema_cx - crop_size / 2.0f);
    const float top = std::max(0.0f, g_ema_cy - crop_size / 2.0f);
    const float right = std::min((float)RAW_FRAME_SIZE, g_ema_cx + crop_size / 2.0f);
    const float bottom = std::min((float)RAW_FRAME_SIZE, g_ema_cy + crop_size / 2.0f);
...
static void equalize_gray_u8(const uint8_t* src, uint8_t* dst, int n){
    ...
    uint8_t lut[256];
    for(int v = 0; v < 256; ++v){
        if(den <= 0){ lut[v] = (uint8_t)v; continue; }
...
bool preprocess_face(const FaceBox& box, float* out_tensor){
    ...
    const float box_size = box.crop_size * 1.1f;
    ...
    for(int ty=0; ty<FACE_TARGET_SIZE; ++ty){
        for(int tx=0; tx<FACE_TARGET_SIZE; ++tx){
            ...
            sx = std::max(0.0f, std::min(sx, (float)RAW_FRAME_SIZE - 1.0f));
            sy = std::max(0.0f, std::min(sy, (float)RAW_FRAME_SIZE - 1.0f));
            int x0 = (int)floorf(sx);
            int y0 = (int)floorf(sy);
            int x1 = std::min(x0 + 1, RAW_FRAME_SIZE - 1);
            int y1 = std::min(y0 + 1, RAW_FRAME_SIZE - 1);
            float wx = sx - x0;
            float wy = sy - y0;
            ...
            int gi = (int)std::max(0.0f, std::min(255.0f, gray));
