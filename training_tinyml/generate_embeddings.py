...
def generate_database():
    ...
    for idx, user_name in enumerate(subdirs):
        ...
        # 3. Tính toán Vector đại diện trung bình (Mean Embedding) và chuẩn hóa L2
        ...
        print(f"    ✅ Đã tạo vector đại diện 128 chiều cho '{user_name}' từ {len(user_embeddings)} ảnh.")
    ...
    cpp_header_path = os.path.join(base_dir, "firmware_esp32", "src", "face_database.h")
    sketch_header_path = os.path.join(base_dir, "firmware_esp32", "src", "tinyml_recognizer", "face_database.h")
    ...
    print(f"[+] Đã tự động xuất mã nguồn C++ CSDL nạp cho ESP32 tại:\n    👉 {sketch_header_path}")
